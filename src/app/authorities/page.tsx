...
export default function AuthoritiesPage() {
  ...
  const handleDownloadDossier = (inst: MonitoredInstitution) => {
    const headers = ['Institution Name', 'Code', 'District', 'Type', 'Overall Score', 'Physical Score', 'Digital Score', 'Human Support Score', 'Exam Score', 'Specially-Abled Students', 'Special Educators', 'Scribes', 'Status', 'Principal / Head', 'Contact Email'];
    const row = [
      inst.name,
      inst.code,
      inst.district,
      ...
      `${inst.physicalScore}%`,
      `${inst.digitalScore}%`,
      `${inst.humanSupportScore}%`,
      `${inst.examScore}%`,
      inst.studentsEnrolled,
      inst.specialEducators,
      inst.verifiedScribes,
      inst.status,
      inst.principalOrHead,
      inst.contactEmail
    ];
    exportToCSV(`Audit_Dossier_${inst.code}`, headers, [row]);
    setSelectedInst(null);
    setToastMessage(`Downloaded complete audit dossier CSV for ${inst.name} (${inst.code})!`);
    setTimeout(() => setToastMessage(null), 5000);
  };

  const handleExportAllDossier = (format: string) => {
    const headers = ['Institution Name', 'Code', 'District', 'Type', 'Overall Score', 'Physical Score', 'Digital Score', 'Human Support Score', 'Exam Score', 'Specially-Abled Students', 'Status', 'Remediation Plan', 'Principal / Head', 'Contact Email'];
    const rows = jharkhandMonitoredInstitutions.map(inst => [
      inst.name,
      inst.code,
      inst.district,
      inst.type,
      ...
      inst.studentsEnrolled,
      inst.status,
      inst.remediationPlan,
      inst.principalOrHead,
      inst.contactEmail
    ]);
    exportToCSV(`Jharkhand_State_Accessibility_Dossier_2026`, headers, rows);
    setShowExportModal(false);
    setToastMessage(`Downloaded State Accessibility Compliance Dossier (${format}) successfully!`);
    setTimeout(() => setToastMessage(null), 5000);
  };
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-[#0f2b5c] border border-cyan-500/40 rounded-2xl max-w-xl w-full p-5 sm:p-6 space-y-4 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-2.5 pt-1">
              ...
              <button
                onClick={() => handleDownloadDossier(selectedInst)}
                className="bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-slate-950 font-black px-4 py-2 rounded-xl text-xs shadow-sm transition flex items-center gap-1.5"
              >
                <Download className="w-4 h-4" />
                <span>{language === 'hi' ? 'पूर्ण ऑडिट डाउनलोड करें' : 'Download Complete Dossier'}</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="bg-[#0f2b5c] border border-cyan-500/40 rounded-2xl max-w-md w-full p-5 sm:p-6 space-y-4 shadow-2xl">
            ...
            <div className="space-y-2">
              {[
                ...
                <div
                  key={i}
                  onClick={() => handleExportAllDossier(item.format)}
