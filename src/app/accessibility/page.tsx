...
export default function AccessibilityServicesPage() {
  ...
  if (isStaff) {
    const allRequests = [
      { id: 'sr-001', studentName: 'Aarav Sharma', supportType: 'Certified Scribe', requestDate: '2026-08-15', examDate: '2026-09-05', status: 'assigned', professional: 'Priya Kumari', priority: 'high' },
      { id: 'sr-002', studentName: 'Ananya Deshmukh', supportType: 'Sign Language Interpreter', requestDate: '2026-08-17', examDate: '2026-09-08', status: 'pending', professional: '—', priority: 'medium' },
      { id: 'sr-003', studentName: 'Rohan Verma', supportType: 'Lesson Reader', requestDate: '2026-08-18', examDate: '2026-09-10', status: 'completed', professional: 'Suresh Yadav', priority: 'low' },
      { id: 'sr-004', studentName: 'Priya Singh', supportType: 'Accessibility Support Assistant', requestDate: '2026-08-19', examDate: '2026-09-12', status: 'pending', professional: '—', priority: 'high' },
    ];

    const filtered = staffStatusFilter === 'all' ? allRequests : allRequests.filter(r => r.status === staffStatusFilter);
    ...
    return (
      <div className="space-y-8 py-2">
        ...
        <div className="bg-[#0f2b5c] border border-cyan-500/30 p-6 rounded-3xl shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <button
            onClick={() => alert(language === 'hi' ? 'सहायता रिपोर्ट डाउनलोड हो रही है...' : language === 'mr' ? 'सहाय्य अहवाल डाउनलोड होत आहे...' : 'Downloading support report...')}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: language === 'hi' ? 'कुल अनुरोध' : language === 'mr' ? 'एकूण विनंत्या' : 'Total Requests', value: allRequests.length, color: 'text-cyan-300' },
              ...
              <div key={req.id} className="bg-[#081a3b] border border-blue-400/20 hover:border-cyan-400/40 p-5 rounded-2xl flex flex-col lg:flex-row lg:items-center justify-between gap-4 transition shadow-md">
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[11px] text-blue-200">
                    ...
                    <div><strong className="text-slate-400">{language === 'hi' ? 'नियुक्त पेशेवर:' : language === 'mr' ? 'नियुक्त व्यावसायिक:' : 'Assigned To:'}</strong> {req.professional}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => alert(`Reviewing request: ${req.id}`)} className="bg-[#0f2b5c] hover:bg-[#123366] text-blue-200 border border-blue-400/30 px-3 py-2 rounded-xl font-semibold text-xs transition">{t.staffActionReview}</button>
                  {req.status === 'pending' && (
                    <button onClick={() => alert(`Assigning professional for: ${req.studentName}`)} className="bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-bold px-3 py-2 rounded-xl transition shadow-md text-xs">{t.staffActionAssignSupport}</button>
                  )}
                  <button onClick={() => alert(`Updating status: ${req.id}`)} className="bg-emerald-700 hover:bg-emerald-600 text-white font-bold px-3 py-2 rounded-xl transition shadow-md text-xs">{t.staffActionUpdateStatus}</button>
                </div>
              </div>
            ))}
          </div>
