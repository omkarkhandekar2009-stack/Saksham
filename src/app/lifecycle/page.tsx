...
export default function LifecyclePage() {
  ...
  const [admissionStatuses, setAdmissionStatuses] = useState<Record<string, string>>({
    'std-001': 'enrolled',
    'std-002': 'enrolled',
    'std-003': 'pending',
  });
  ...
  if (isStaff) {
    ...
    const accessibilityRequired = students.filter(s => s.accessibilityProfile.categories.length > 0).length;
    const studentsPlaced = 2;
    const placementPending = students.length - studentsPlaced;

    const mockAdmissions = [
      { id: 'adm-001', studentName: 'Ravi Kumar', grade: 'Grade 9', disability: 'Visual Impairment (80%)', status: 'pending', accessNeeds: 'Braille books, Screen reader', date: '2026-08-15' },
      { id: 'adm-002', studentName: 'Sunita Devi', grade: 'Grade 10', disability: 'Hearing Impairment (75%)', status: 'under_review', accessNeeds: 'Sign language interpreter', date: '2026-08-17' },
      { id: 'adm-003', studentName: 'Mohan Prasad', grade: 'Grade 11', disability: 'Mobility (60%)', status: 'approved', accessNeeds: 'Wheelchair ramp, Accessible toilet', date: '2026-08-18' },
    ];
    ...
    const statusColor = (status: string) => {
      if (status === 'approved' || status === 'placed') return 'bg-emerald-500/20 text-emerald-300 border-emerald-400/40';
      ...
      <div className="space-y-8 py-2">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#0f2b5c] border border-cyan-500/30 p-6 rounded-3xl shadow-xl">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => alert(language === 'hi' ? 'प्रवेश रिपोर्ट डाउनलोड हो रही है...' : language === 'mr' ? 'प्रवेश अहवाल डाउनलोड होत आहे...' : 'Downloading admission report...')}
                ...
                <div key={adm.id} className="bg-[#081a3b] border border-blue-400/20 hover:border-cyan-400/40 p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 transition shadow-md">
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-blue-200">
                      ...
                      <div><strong className="text-slate-400">{language === 'hi' ? 'आवश्यकताएं:' : language === 'mr' ? 'आवश्यकता:' : 'Accessibility Needs:'}</strong> {adm.accessNeeds}</div>
                      <div><strong className="text-slate-400">{language === 'hi' ? 'आवेदन तिथि:' : language === 'mr' ? 'अर्ज तारीख:' : 'Applied:'}</strong> {adm.date}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => alert(`Reviewing: ${adm.studentName}`)} className="bg-[#0f2b5c] hover:bg-[#123366] text-blue-200 border border-blue-400/30 px-3 py-2 rounded-xl font-semibold text-xs transition">{t.staffActionReview}</button>
                    {adm.status === 'pending' && (
                      <button onClick={() => alert(`Updating status for: ${adm.studentName}`)} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3 py-2 rounded-xl transition shadow-md text-xs">{t.staffActionUpdateStatus}</button>
                    )}
                    <button onClick={() => alert(`Assigning support for: ${adm.studentName}`)} className="bg-cyan-700 hover:bg-cyan-600 text-white font-bold px-3 py-2 rounded-xl transition shadow-md text-xs">{t.staffActionAssignSupport}</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-4">
              ...
              {mockPlacements.map((pl) => (
                <div key={pl.id} className="bg-[#081a3b] border border-blue-400/20 hover:border-cyan-400/40 p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 transition shadow-md">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => alert(`Viewing placement: ${pl.studentName}`)} className="bg-[#0f2b5c] hover:bg-[#123366] text-blue-200 border border-blue-400/30 px-3 py-2 rounded-xl font-semibold text-xs transition">{t.staffActionViewPlacement}</button>
                    {pl.status === 'pending' && (
                      <button onClick={() => alert(`Recording placement for: ${pl.studentName}`)} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3 py-2 rounded-xl transition shadow-md text-xs">{t.staffActionRecordPlacement}</button>
                    )}
                  </div>
                </div>
              ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {mockJobs.map(job => (
                    <div key={job.id} className="bg-[#051124] border border-blue-400/20 p-4 rounded-2xl space-y-2">
                      ...
                      <button onClick={() => alert(`Referring student to: ${job.company}`)} className="w-full bg-blue-700 hover:bg-blue-600 text-white text-xs font-bold py-2 rounded-xl transition">
